...
const Ourachievement = () => {
  ...
  const tabs = [
    { name: "Achievements ISO Certified", content: "No data for this category",icon:<FaTrophy/> },
    { name: "Trade License Certified", content: "No data for this category",icon:<FaTrophy/> },
    { name: "Evolution Partner Certificate", content: "No data for this category",icon:<FaHeadset/> },
    { name: "Casino Malaysia License", content: "No data for this category" ,icon:<FaMoneyBillWave/> },
    { name: "Casino Partner Certificate", content: "No data for this category",icon:<FaChartBar/> },
    { name: "Betfair Api Certificate", content: "No data for this category",icon:<FaUserFriends/> },
    { name: "Gaming Curacao Certificate", content: "No data for this category",icon:<FaTrophy/> },
  ];
  const [activeTab, setActiveTab] = useState(0);
  const [achievement,set_achievement]=useState([]);
    const get_achievement=()=>{
        axios.get(`${base_url}/admin/all-achievement`)
        .then((res)=>{
            if(res.data.success){
...
<div className="mx-auto flex max-w-[58rem] flex-col items-center space-y-4 text-center">
    <h2 className="font-bold text-3xl leading-[1.1] sm:text-3xl md:text-[30px] text-white">Our Acievement</h2>
  ...
  <div className="mt-[30px] lg:mt-[50px] font-poppins grid justify-center gap-4 grid-cols-1 sm:grid-cols-2   w-full md:grid-cols-4 ">
   {
    achievement?.map((data,i)=>{
        return(
          <Achievementmodel data={data} key={i}/>
        )
    })
   }
