  ...
  const deleteDeposit = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${base_url}/admin/delete-deposit/${id}`, {
              ...
              <table className="w-full border-collapse border-[1px] border-[#eee]">
                <thead>
                  <tr className="bg-indigo-500 text-white">
                    ...
                    <th className="py-3 px-4 text-left">Sender Number</th>
                ...
                <tbody>
                  {filteredDeposits.map((deposit) => (
                    <tr key={deposit._id} className="border-b">
                      <td className="py-3 px-4 text-orange-500 font-[500]">{deposit.invoiceId}</td>
                      <td className="py-3 px-4">{new Date(deposit.createdAt).toLocaleDateString()}</td>
                      <td className="py-3 px-4">{deposit.amount}</td>
                      <td className="py-3 px-4">{deposit.gatewayName}</td>
                      <td className="py-3 px-4">{deposit.senderNumber}</td>
