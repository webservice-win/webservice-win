...
const [courses,set_courses]=useState([]);
    ...
    course.title.toString().includes(searchQuery) ||
    course.details.toString().includes(searchQuery) 
  );
      // ------------delete course-------------
            const delete_course=(id)=>{
      const confirm_box=confirm("Are you sure?");
       if(confirm_box){
       axios.delete(`${base_url}/admin/delete-accordion/${id}`, {
                headers: {
  ...
  return (
    <section className='w-full h-[100vh] flex font-poppins'>
       ...
       <div className='w-full flex justify-between items-center'>
        <div>
              <h1 className='text-[20px] lg:text-[20px] font-[600] mb-[8px]'>Accordions List</h1>
          <ul className='flex justify-center items-center gap-[10px] text-neutral-500 text-[14px] font-[500]'>
             <li>Accordions</li>
            <li><IoIosArrowForward/></li>
            <li>Accordions List</li>
          </ul>
        </div>
        {/* -------------search-box------------------ */}
   {
        ...
        courses.length > 0 ?     <div className="relative overflow-x-auto border-[1px] border-[#eee] rounded-[5px]">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border-l border-r border-b border-gray-200 dark:border-gray-700">
            <thead className="text-xs text-white uppercase bg-indigo-500 dark:bg-gray-700 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
              <tr>
                <th scope="col" className="px-6 py-[15px] border-r border-gray-200 dark:border-gray-700 text-[15px] font-[500] text-nowrap">Title</th>
                <th scope="col" className="px-6 py-[15px] border-r border-gray-200 dark:border-gray-700 text-[15px] font-[500] text-nowrap">Details</th>
            ...
            <tbody>
              {filteredCourses.map((data, index) => (
                <tr key={index} className="bg-white border-b font-[500] border-gray-200 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
      
                  <th scope="row" className="px-6 py-2 text-[16px] font-[500] text-neutral-700 whitespace-nowrap dark:text-white border-r border-gray-200 dark:border-gray-700">
                    {data.title.length > 30 ? <p>{data.title.slice(0,30)}...</p>:<p>{data.title}</p>}
                  </th>
                  <th scope="row" className="px-6 py-2 text-[16px] font-[500] text-neutral-700 whitespace-nowrap dark:text-white border-r border-gray-200 dark:border-gray-700">
                   {data.details.length > 30 ? <p>{data.details.slice(0,30)}...</p>:<p>{data.details}</p>}
                  </th>
                  <th scope="row" className="px-6 py-2 text-[16px] font-[500] text-neutral-700 whitespace-nowrap dark:text-white border-r border-gray-200 dark:border-gray-700">
                  ...
                  <td className="px-6 py-2 flex justify-start items-center gap-[8px]">
                    <NavLink to={`/accordion/edit-accordion/${data._id}`} className="font-medium text-white dark:text-blue-500 hover:underline p-[10px] text-[22px] cursor-pointer bg-indigo-500 rounded-[5px]">
                      <FiEdit />
                    </NavLink>
