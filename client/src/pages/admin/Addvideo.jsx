...
//   ------------------handle-form--------------
 ...
 const [form, setForm] = useState({
  ...
  const [selcect_category,set_category]=useState("");
...
const handleSubmit = async (e) => {
  ...
  formData.append("category", selcect_category);
  if (file) {
  ...
  Swal.fire({
    title: "Submitting...",
    html: "Please wait while your video information is being submitted.",
  ...
  try {
    ...
    const response = await axios.post(`${base_url}/admin/add-video`, formData, {
      headers: {
    ...
    if (response.status === 200) {
      setForm({ videoUrl: "", profileImage: null }); // Reset form data
  ...
  return (
    <section className='w-full h-[100vh] flex font-poppins'>
       ...
       {/* ------------------new category----------------- */}
         <section className='pt-[15px] px-[20px] pb-[20px] w-full border-[1px] border-[#eee]  mt-[20px] rounded-[5px]'>
           <div>
                <h2 className='text-[20px]  mb-[15px] lg:text-[20px] font-[600]'>Add Video</h2>
            </div>
            {/* -------------------form---------------------- */}
 ...
 <div className="w-full flex gap-[10px] lg:gap-[30px] mb-[10px]">
        ...
        <select name="category" id=""onChange={(e)=>{set_category(e.target.value)}}  className="w-full mt-[8px] rounded-[5px] placeholder-gray-500 outline-brand_color text-[14px] h-[45px] border-[1px] border-[#eee] px-[15px]">
