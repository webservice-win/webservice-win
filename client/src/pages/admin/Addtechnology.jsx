...
const [category,set_category]=useState([]);
    const get_category=()=>{
        axios.get(`${base_url}/admin/all-technology`, {
                headers: {
...
const handleSubmit = async (e) => {
  ...
  Swal.fire({
    title: "Submitting...",
    html: "Please wait while your technology data is being submitted.",
  ...
  try {
    ...
    const response = await axios.post(`${base_url}/admin/add-technology`, formData, {
      headers: {
        'Authorization': localStorage.getItem('token'),
      },
    });
    ...
    if (response.status === 200) {
      ...
      get_category(); // Update categories or any other necessary action
    } else {
    ...
    // ------------delete category-------------
      ...
      const confirm_box=confirm("Are you sure?");
       if(confirm_box){
       axios.delete(`${base_url}/admin/delete-technology/${id}`, {
                headers: {
                    'Authorization': localStorage.getItem('token')
                }
            })
    ...
    <section className='w-full h-[100vh] flex font-poppins'>
       ...
       {/* ------------------new category----------------- */}
         <section className='pt-[40px] pb-[30px] w-full border-[1px] border-[#eee] shadow-sm px-[20px] mt-[20px] rounded-[5px]'>
            <div>
                <h2 className='text-[20px] lg:text-[20px] font-[600]'>Add Technology</h2>
            </div>
            {/* -------------------form---------------------- */}
      ...
      <div className="w-full flex justify-between items-center mb-[20px] flex-col lg:flex-row gap-[15px]">
        <h1 className="w-full text-left text-[20px] lg:mb-[15px] font-[600]">Technology List</h1>
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          <thead className="text-xs text-white uppercase bg-[#6366F1] dark:bg-gray-700 dark:text-gray-400">
            <tr>
              ...
              <th scope="col" className="px-6 py-4 border-r border-gray-200 dark:border-gray-700  text-[14px] lg:text-[16px] font-[600]">
                Action
              </th>
            </tr>
          </thead>
