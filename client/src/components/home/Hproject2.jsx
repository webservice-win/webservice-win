...
const [websites,set_websites]=useState([]);
 ...
 <div className='flex justify-center items-center'>
          ...
          websites.map((data,i)=>{
            return(
               <div className='p-[5px] bg-white rounded-[10px]'>
                 <div key={i} className='p-[7px] h-[100%] rounded-[10px]  font-poppins group bg-[#010053] overflow-hidden '>
     ...
     {
        data.title.length >60 ?  <NavLink to={`/single-website-details/${data._id}`}><h2 className='text-white font-noto-sans text-[16px] hover:underline hover:text-orange-400 cursor-pointer  lg:text-[22px] mb-[15px] font-[600] '>{data.title.slice(0,60)}...</h2></NavLink>:<NavLink to={`/single-website-details/${data._id}`}> <h2 className='text-[16px] lg:text-[22px] mb-[5px] lg:mb-[15px] hover:underline hover:text-orange-400 font-[600] font-noto-sans text-white'>{data.title}</h2></NavLink>
       }
                       <div className='mb-[8px] flex gap-[10px] justify-between items-center '>
<h2 className='text-[12px] lg:text-[13px] p-[10px] rounded-full  bg-indigo-800 font-[500] text-white'>Single License:${data.singleLicense}</h2>
                       <h2 className='text-[12px] lg:text-[13px] p-[10px] rounded-full bg-indigo-800 font-[500] text-white'>Unlimited License: ${data.unlimitedLicense}</h2>
                    ...
                    <NavLink to={`${data.demoFrontend}`} className="w-[50%]" target='_blank'>
                       <button className='w-full py-[8px] lg:h-[50px] text-white bg-[#2563EB] font-[500] rounded-full text-[14px] lg:text-[19px] font-[800] hover:bg-red-400 hover:text-white transition-all duration-150 font-bangla_font'>লাইভ ডেমো</button>
                    </NavLink>
                        <NavLink to={`/single-website-details/${data._id}`} className="w-[50%]">
                                <button className='w-full py-[8px] lg:h-[50px] text-white bg-[#2563EB] font-[500] rounded-full text-[14px] lg:text-[19px] hover:bg-red-400 hover:text-white font-[800] transition-all duration-150 flex font-bangla_font justify-center items-center gap-[8px]'>বিস্তারিত দেখুন<FaArrowUpRightFromSquare/></button>
                        </NavLink>
                    </div>
                </div>
              </div>
   ...
   <div className="flex w-full items-center justify-center mt-[50px] z-[100]">
              <button className='px-[30px] py-[12px] rounded-full text-white bg-indigo-800 border-[2px] border-white font-poppins text-[16px]'>Load more</button>
         </div>
